...
var isvalid = true;
app.controller('SalesCntr', ['$scope', '$http', '$window', '$rootScope', 'Excel', '$timeout', function ($scope, $http, $window, $rootScope, Excel, $timeout) {
    
    $scope.AccReport = SaleProdReportDO();
    ...
    $scope.AccReport.InTermsOf = $("#select").val();
    ...
    $scope.GetReportDetails = function () {
        ...
        PopUpController.OpenPopup1("divPopup", "SalePerson Productivity Report", popupdetail);
        $scope.ReportFilter.CustomDateOption = $("#ddl_CustomDateRange").val();
       
        $scope.SalesProductivityList = [];
        $http({
            method: "post",
            url: "../Production/GetSalesProductivityDetail",
            data: ({
            ...
            if (d.data != undefined || d.data != null) {
                if ((d.data).length > 0) {
                    ...
                    $scope.SalesProductivityList = d.data;
                    $scope.ModifySearch = false;
